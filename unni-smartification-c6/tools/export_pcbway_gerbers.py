#!/usr/bin/env python3
"""Export the two physical PCBs from the combined KiCad board as PCBWay-ready ZIPs.
...
Usage:
  ...
  KICAD_10_0_5_CLI=/path/to/kicad-cli python3 tools/export_pcbway_gerbers.py
  python3 tools/export_pcbway_gerbers.py --output /tmp/pcbway --keep-temp

The output ZIP contents are at ZIP root so they can be uploaded directly to
PCBWay. Paste layers are intentionally omitted; they belong to stencil/PCBA
...
def fail(msg: str) -> None:
    raise SystemExit(f"PCBWAY EXPORT ERROR: {msg}")
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "dist" / "pcbway")
    ...
    tmp_obj = tempfile.TemporaryDirectory(prefix="unni-pcbway-")
    ...
    for name, rect in boards.items():
        ...
        run([
            ...
            "--check-zones", "--no-x2", "--no-netlist", "-o", fab, sliced,
        ])
        run([
        ...
        archive = output / f"{stem}-pcbway-gerbers.zip"
        zip_dir(fab, archive)
    ...
    (output / "README.txt").write_text(
        "PCBWay fabrication exports\n"
        "==========================\n\n"
        "Generated from the combined two-island KiCad 10.99 master via a temporary KiCad 10.0.x compatibility copy.\n"
        "Target order: FR-4, 4 layers, 1.0 mm, 1 oz finished outer copper, 1 oz inner copper.\n"
        "Gerbers are emitted as classic RS-274-X (X2/netlist attributes disabled) with Protel extensions.\n"
