# Design decisions — Rev A work in progress
...
- Charger is MCP73831T-2ACI/OT with 5.1 kΩ PROG (~196 mA nominal), operating autonomously whenever VBUS is present. This avoids a flat-battery deadlock where an unpowered ESP would have to enable charging and reduces USB budget/charger dissipation.
...
Redundant connector alias nets were removed: GND, VBUS, USB_RAW_P, USB_RAW_N and +BATT are used directly. With the shunt-style TPD2E009DBZR, J1, U4 and the downstream USB pair share USB_RAW_P/USB_RAW_N; there is no artificial pre-/post-ESD net split.

The ESP sheet is now included in SPICE because MCP73831 and TPS63031 live there.  Physical connectors and the ESP32-C6 module are explicitly excluded from simulation where required, preserving the complete power-stage simulation without placeholder devices.

## Rev A power/USB/fabrication decisions (2026-08-23)

- The battery pack is a protected 1S Li-ion pack; duplicate cell-protection FETs are intentionally not added to the PCB.
- MCP73831 charge current is reduced from ~300 mA to ~196 mA with RPROG = 5.1 kΩ to reduce USB input budget and SOT-23 charger dissipation.
