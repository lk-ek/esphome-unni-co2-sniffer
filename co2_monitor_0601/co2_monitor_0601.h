...
class CO2Monitor0601 : public Component {
 public:
  ...
  void set_unni_display_humidity_sensor(sensor::Sensor *s) { this->out_.display_humidity = s; }
 ...
 protected:
  struct Outputs {
    ...
    sensor::Sensor *display_humidity{nullptr};
  ...
  struct HaState {
    ...
    bool have_display_humidity{false};
    ...
    bool initial_display_humidity_published{false};
    float co2{0.0f};
  ...
  bool wifi_disable_pending_{false};
  uint32_t wifi_disable_requested_ms_{0};
  uint32_t wifi_recovery_until_ms_{0};
  uint32_t wifi_recovery_window_ms_{300000};
