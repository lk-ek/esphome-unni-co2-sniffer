...
The ESP32-C3 RMT receiver now records the SCL pulse timeline independently of the shared GPIO ISR. The GPIO capture remains authoritative for SDA and for the downloadable raw trace, while RMT can restore an SCL transition pair that the CPU-side edge ISR missed completely. A second decoder safeguard resolves the observed case where a delayed GPIO ISR collapses an SDA setup transition and the following SCL rise into one sample. RMT repair is deliberately conservative: if the GPIO and RMT timelines cannot be aligned unambiguously, the code falls back to the untouched GPIO capture instead of guessing. The RMT channel is disabled whenever battery power policy disables CO₂ capture.

The first malformed, otherwise unhandled, protocol-invalid, RMT-repaired, or coalesced-edge CO₂ transaction freezes its **original GPIO** raw edge capture so that later normal traffic cannot overwrite the interesting waveform. Download `/capture` to retrieve that frozen trace; only a successful, complete HTTP transfer releases the freeze, so a disconnected or timed-out client can simply retry. Debug downloads are streamed in moderate chunks to avoid large one-shot socket writes. New captures use the `LA02` format, which preserves the bus state before the first edge. `tools/capture2vcd.py` reads `LA02` and both historical `LA01` header variants.
